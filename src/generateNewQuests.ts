...
export default async function generateNewQuests(plugin: ResearchQuest) {
  ...
  try {
    ...
    if (numQuestsNeeded > 0) {
      const newQuestions = await plugin.openai.generateQuestions(fileContent, numQuestsNeeded);
    ...
    if (currentActiveQuests.length > 0) {
      ...
      const updated = quests.map(quest => {
        const evaluation = evaluations.evaluations.find(e => e.questionId === quest.id);
